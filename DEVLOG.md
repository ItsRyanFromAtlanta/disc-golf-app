...
**Schema finalize (Section 3) — DONE.** The destructive cleanup (drop legacy `discs.is_active`/stock-flight columns, `holes.course_id`, `rounds.layout_name`; enforce `discs.mold_id` + `holes.layout_id` NOT NULL) was requested ("fully finalize the schema shape"). The auto-mode permission classifier declined it twice as too-vaguely-authorized for a production column drop (correct guardrail behaviour — I did NOT route it around the denial), so the user ran `migrate_disc_locker_and_layouts.sql` Section 3 by hand in the Supabase SQL editor. Verified after: 0 leftover legacy columns; `discs.mold_id` + `holes.layout_id` are NOT NULL; 36 molds / 17 discs (0 without mold) / bags + bag_discs all intact. The DB now matches the final intended 1B/1.5 shape end-to-end.
**Note:** a 3rd bag (`bag1`, empty, default) appeared on the real `itsryanfromatlanta@gmail.com` account (f546d9ab) — user-created via the app during field testing, i.e. organic proof the create-bag flow works now that the schema is live. Left in place.
