...
| Disc universe: full ingestion pipeline (1F) | BACKLOG | B1.6 pure repository/adapter contracts are shipped; network/server ingestion remains deferred: user-provided URL hints, raw payload staging, dedup review queue, open-dataset check, image URLs only, and polite crawling |
