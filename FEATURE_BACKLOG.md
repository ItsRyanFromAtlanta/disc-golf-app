...
| Browser E2E baseline | IN PROGRESS | Harness shipped 2026-07-28: Playwright, `e2e/` with 25 specs over two viewport projects, authenticated via seeded session with the backend intercepted in-page, plus an `e2e` CI job. Covers tab scroll/root, notification sheet/Back, 320px reflow, route guards, offline shell boot, keyboard tab operation, soft-delete/restore, and resume-to-capture-route. Still uncovered: single-active auto-close, round-close confirmation, completed edit/audit, exactly-once reconnect — all need a live capture session rather than a seeded one. Coverage table in `PHASE_A_ARCHITECTURE.md` § 9; harness limits in `e2e/README.md` |
...
| Live-capture E2E fixtures | SHIPPED | 2026-07-28. `capture.spec.js` drives a real session through the launcher's Start button — the only browser path to `planActivityStart`. Suite at 31 specs. Moved pause/navigation/resume to Covered and the other four § 9 rows to honest Partial |
| Reconnect double-send in syncScheduler | NEXT UP | `handleOnline`/`handleVisibility` guard only on `status !== FAILED` while `notifyOutboxChanged` guards on `!== SYNCING`, so an `online` event can start a second concurrent flush and every queued operation is sent twice (~1 run in 3). Idempotency keys absorb it server-side, so this is wasted reconnect traffic rather than corruption — but it defeats wire-level exactly-once. Found by the reconnect spec 2026-07-28 |
| History Detail "Saved" confirmation never shows | NEXT UP | `SessionReport` keys `NotesTagsEditor` on `notes`/`tags`, so a successful save changes the key and remounts the editor, discarding the confirmation. Found 2026-07-28 |
| ChipGroup selection is not exposed to assistive tech | NEXT UP | Selection is a CSS class only — no `aria-pressed` — so a selected tag cannot be announced or asserted by role and name. Found 2026-07-28 |
