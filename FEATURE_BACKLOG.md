...
| Honest `reason` on round-start bag snapshots | BACKLOG | `captureBagVersion` defaults `reason` to `'grouped_save'` and `roundRepository` does not override it, so a snapshot taken because a round started claims to have been taken because the bag was saved. The `bag_versions.reason` CHECK already allows four values but `capture_bag_version` accepts two, so an honest `'round_start'` needs a migration widening both. Verification does not depend on it — it reads the timeline, not the labels. Recorded as F3 in the E2 audit |
