...
| Browser E2E baseline | IN PROGRESS | Harness shipped 2026-07-28: Playwright, `e2e/` with 19 specs over two viewport projects, authenticated via seeded session with the backend intercepted in-page, plus an `e2e` CI job. Covers tab scroll/root, notification sheet/Back, 320px reflow, route guards, offline shell boot, keyboard tab operation. Still uncovered: pause/resume, single-active auto-close, round-close confirmation, completed edit/audit, soft-delete/restore, exactly-once reconnect — all need activity-lifecycle fixtures. Coverage table in `PHASE_A_ARCHITECTURE.md` § 9; harness limits in `e2e/README.md` |
| Activity-lifecycle E2E fixtures | NEXT UP | The six uncovered § 9 flows all need an in-progress/completed activity seeded through the InstantLaunch + Dexie layers, not just table rows. Next increment on the existing harness |
