...
| Disc universe: MVP/Axiom/Streamline + Innova seed | IN PROGRESS | Manufacturer-site import with attribution (source_url/source_name/scraped_at on disc_molds); bounded official MVP snapshot adapter now covers four representative molds; continue under the current Codex roadmap |
| Disc universe: full ingestion pipeline (1F) | IN PROGRESS | B1.7/B1.8 server-only staging, official MVP adapter composition, bounded product-page fetch/parser, review, atomic dependency-ordered promotion, provenance, raw-artifact guard, admin JWT boundary, bounded non-production fixture, and bounded official MVP source adapter are applied; open-dataset check found no safe candidate for promotion; remaining work is backup-gated transactional staging RPC/store, protected Edge Function, conditional fetch state, and polite crawler/scheduler/admin UI |
