...
--
-- IDEAL FORMAT: this seeds the `badges` REFERENCE table (created in
-- layer1_foundation_schema.sql: code unique, name, description, tier, criteria
-- jsonb). It is generated to match src/lib/gamification/badgeCatalog.js exactly
-- — that JS catalog is the single source of truth (code/name/description/tier/
-- criteria), this file is its DB projection. criteria.metric keys into the pure
-- metric registry in src/lib/gamification/metrics.js.
--
-- Icons are intentionally NOT stored here — they are presentation, not criteria;
-- the UI maps code -> emoji via BADGE_ICONS in badgeCatalog.js.
--
-- TEST DATA NOTE: per-user badge_progress / xp_events for the exercised test
-- account (35b59d46…) are deliberately NOT hand-written here. They are produced
-- by running the real evaluator (evaluateAndPersistBadges) against that account's
-- actual history during browser verification — the only way to guarantee the
-- seeded progress is exactly what production would compute, rather than a
-- hand-transcribed copy that could drift from the engine. See DEVLOG.
--
-- IDEMPOTENT: ON CONFLICT (code) DO UPDATE, so re-running re-syncs definitions to
-- the catalog without duplicating rows or disturbing existing badge_progress FKs.
