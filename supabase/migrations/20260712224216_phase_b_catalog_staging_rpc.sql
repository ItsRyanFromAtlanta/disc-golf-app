-- Phase B B1.9 transactional catalog staging.
--
-- Ideal format:
--   * catalog_ensure_source resolves the existing expression-indexed source
--     identity or creates it idempotently for the server-only store.
--   * catalog_stage_import atomically inserts a staged batch, its immutable
--     raw-artifact metadata, and its normalized candidates.
--   * the staging transaction checks the explicit ingestion-admin allowlist,
--     never writes canonical catalog tables, and is executable only by
--     service_role.
--   * repeated source/adapter/version/checksum tuples return the existing
--     batch without duplicating artifacts or candidates.
...
begin
  ...
  select s.id
   ...
   where s.source_type = p_source_type
     ...
     and pg_catalog.lower(pg_catalog.coalesce(pg_catalog.btrim(s.url), ''))
...
begin
  ...
  v_artifact_kind := pg_catalog.coalesce(p_artifact ->> 'artifact_kind', 'raw_response');
  v_storage_bucket := pg_catalog.coalesce(p_artifact ->> 'storage_bucket', 'catalog-import-raw');
  ...
  if v_artifact_checksum is distinct from v_source_checksum
     ...
     or (p_artifact ->> 'http_status') !~ '^[0-9]+$'
     or (p_artifact ->> 'response_bytes') !~ '^[0-9]+$'
  then
    raise exception using errcode = 'P0001', message = 'catalog_stage_request_invalid';
  end if;
  ...
  values (
    ...
    pg_catalog.coalesce(nullif(p_artifact ->> 'redirect_count', '')::smallint, 0),
...
revoke all on function public.catalog_ensure_source(text, text, text)
  from public, anon, authenticated;
grant execute on function public.catalog_ensure_source(text, text, text)
  to service_role;

revoke all on function public.catalog_stage_import(uuid, uuid, text, jsonb, jsonb, jsonb)
  from public, anon, authenticated;
grant execute on function public.catalog_stage_import(uuid, uuid, text, jsonb, jsonb, jsonb)
  to service_role;
