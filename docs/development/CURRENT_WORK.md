...
- **Current checkpoint:** B1.7 server-only fetch and staging contracts are implemented and verified;
  B1.7 design and B1.6 repository/manufacturer-adapter contracts remain approved.
...
- **Current implementation:** B1.7 now has server-only fetch policy primitives and a dependency-injected
  staging orchestrator with no Supabase or canonical-write dependency. It enforces lowercase adapter
  slugs, allowlisted HTTPS sources, response/redirect limits, checksummed fetch envelopes, idempotent
  existing-batch replay, and explicit 304 handling. Staged candidates still require a future append-only
  candidate/artifact schema, and canonical promotion remains a separate admin-authorized transaction.
  No Edge Function entrypoint or canonical write path exists yet. B1.6 contract slice remains complete:
  ...
  the runtime model label remains GPT-5 Codex. Candidate persistence and canonical promotion still need
  their own migration/security design reviews.
...
- **Verification for this checkpoint:** 359 unit tests pass, including 26 focused catalog/ingestion
  ...
  333 tests; the current full suite is 359. Browser verification now runs with
...
- **Context recommendation:** the next work is a separately reviewed candidate/artifact migration and
  server persistence adapter. Canonical promotion still requires its own authorization, migration,
  security, backup, and rollback review. Do not add canonical catalog writes to staging. The verified
  pre-apply archive for B1.5 is outside Git at
